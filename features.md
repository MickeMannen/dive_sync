...
| 1.2 | Adapter interface (`BaseDiveAdapter`: login / fetch / add / update / delete) | ✅ | `adapter.py` |
...
| 1.7 | Three-tier dive matching: external-ID link → dive number → timestamp within grace window (default 15 min) | ✅ | `sync_engine.py:match_dives` |
...
| 1.11 | Field-level update of matched dives: buddy, notes, weight (+unit), visibility (+unit) | ✅ | Both directions |
| 1.12 | GPS coordinates sync | 🚧 | Garmin→Divelogs always; Divelogs→Garmin only when Garmin has no coordinates yet |
| 1.13 | Depth/temperature profile samples sync | 🚧 | Garmin→Divelogs only (Garmin activity details + chart metrics) |
| 1.14 | Gas mixtures / tank pressures / tank volume sync | 🚧 | Garmin→Divelogs only. Divelogs→Garmin gas/tank updates are not supported (Garmin's gas structures are restricted). `sync_gases` flag can disable entirely |
...
| 2.2 | Cron-like jobs: hourly / daily / weekly / custom-minute interval | ✅ | `CronJobModel` — per-job direction, only_new, sync_gases, sync_fit, enabled |
...
| 4.6 | Default sync settings form (direction, grace window, cooldown, only_new, sync_gases, sync_fit) | ✅ | `GET/POST /api/settings` |
...
| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 6.1 | Pytest suite (`./run_tests.sh`): sync logic, mock sync, scheduler, dive cache, web API, desktop credentials and preferences | 🚧 | 68 tests across 7 files; 4 in `test_mock_sync.py` skip unless real downloaded fixtures `488.json`/`502.json` exist under `data/` |
...
- In bidirectional mode Garmin silently wins every field conflict (buddy, notes, weight, visibility): the Garmin→Divelogs pass runs first and overwrites the Divelogs values before the reverse pass compares them. Location is never compared on matched dives. There is no per-field direction or conflict setting.
...
| B14 | Generalise `SyncEngine` from a fixed Garmin/Divelogs pair to any two adapters with a `service_id` | | 📝 | Prerequisite for B7/B13; rework.md F1, done together with the B11 refactor |
...
| B17 | Mapping board extras: ordered match keys chosen on the board, read-only 'Test mapping' against the newest 10 live dives, apply-to-all prompt after saving, reset to defaults | | 📝 | rework.md C19, C20; decided 2026-09-21 |
