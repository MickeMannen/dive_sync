...
Last verified against the code: 2026-09-21 (branch `feature_branch`, after phase 2 of the suggested order). Test baseline: `./run_tests.sh` → 120 passed, 4 skipped (the skips in `test_mock_sync.py` need real downloaded fixtures `488.json`/`502.json` under `data/`, which are not committed).
...
| C17 | `field_catalog()` on `BaseDiveAdapter`, implemented by Garmin, Divelogs and both mock adapters, with labels, units, `max_length` and writability; `UnifiedDive.service_fields` filled by `to_unified` and pushed by `update_dive` (Garmin `activityName`/`locationName`, Divelogs `location`/`divesite` stop being collapsed into `location`); new v1 fields rating, water/dive type, tags; `GET /api/fields` returns the catalogues per pair | 🚧 | 2026-09-21: catalogues (17 Garmin, 15 Divelogs fields), `service_id`/`display_name` on every adapter, `service_fields` filled and pushed (Divelogs `location`/`divesite` now round-trip exactly instead of via the comma split), `GET /api/fields` and `POST /api/fields/preview` done. Phase 2 added `fetch_recent_dives(limit)` to the adapter interface (Garmin only fetches details for the newest N). **Still open:** rating / water type / dive type / tags: neither raw payload carries them (checked against a real Garmin and Divelogs dive on 2026-09-21), needs an API-docs check before adding; `max_length` is unknown for every text field and left empty. The `location` collapse in `to_unified` is now the documented *fallback* for uploads (see decisions), not a leftover |
| C18 | Template renderer in `src/core/templates.py`: `{key}` placeholders with format specs, validation against the catalogue, `preview(link, dive)`; truncation to `max_length` with warning; loop detection across links; tests for empty sources, format specs, unknown keys, non-text targets, truncation, loops | ✅ | 2026-09-21. `render()`, `validate_template()`, `detect_loops()`, `validate_links()` (the full Save check, used by the engine, `POST /api/settings` and the CLI), `preview()` with built-in example dives. Bare `{name}` placeholders resolve on the link's source service, so `{dive_number:03d}` works as in the plan. A templated link may only write its target. Composites run on matched pairs and on uploads of new dives (`SyncEngine.prepare_upload()`); the site-name collapse stays as the fallback when no link fills the native fields. Tests in `tests/test_templates.py` and `tests/test_link_engine.py` |
...
| C20 | Test mapping: `POST /api/mapping/test` takes an unsaved board, fetches the newest 10 dives per side with the normal cooldown, returns per-dive source/target/result/conflict rows; strictly read-only; the same core function serves the QML board | 🚧 | Core function `SyncEngine.test_mapping(field_links, limit)` done 2026-09-21 (used by the CLI); the HTTP endpoint is left for the board work in C6 |
...
| C4 | `manual` policy: `conflicts.json` writer/reader in `src/core/conflicts.py`, engine skips the field and records it; `resolve_conflict()` pushes the chosen side | ✅ | 2026-09-21. `Conflict` / `ConflictStore`; file is `conflicts.json` beside `sync_state.json` (same account suffix). Every run replaces the entries of the matched pairs it saw, so stale conflicts vanish; dry runs report but never write. `SyncEngine.resolve_conflict(id, 'source'|'target')` fetches the losing dive (two-day window around the dive, adapters have no get-by-id), writes the recorded value through `update_dive`, drops the entry. Results carry `conflicts` |
| C5 | CLI: `--list-conflicts`, `--resolve <id> source|target`, `--show-mapping` (links per pair as a table), `--validate-profile <file>` (same checks as Save), `--test-mapping` (the 10-dive read-only table); `run_sync_thread` passes `field_links` overrides from cron jobs | ✅ | 2026-09-21. All flags in `sync.py` (plus `--export-profile` / `--import-profile [--yes]` from C9); `--test-mapping` calls `SyncEngine.test_mapping()`, the same function C20 will expose over HTTP. Scheduler half: `run_sync_thread` passes every cron-job value (direction, filters, `field_links`) as explicit `run_sync(...)` arguments, which also fixed a pre-existing bug where `run_sync`'s settings reload discarded per-job values. Subprocess tests in `tests/test_cli.py` |
...
| C9 | Sync profile export/import in `src/core/config.py` with version check and round-trip tests; CLI `--export-profile <path>` / `--import-profile <path>` | ✅ | 2026-09-21. `export_profile()` / `import_profile()` / `write_profile()` / `read_profile()`, `PROFILE_VERSION = 1`, `ProfileImportSummary` (sections replaced, diff lines, skipped links by id, ignored keys). Newer version refused with a plain message; sections present in the file replace the current ones wholesale, absent ones are kept. Tests in `tests/test_profile.py` |
...
| 2026-09-21 | Links on uploads of new dives (phase 2 implementation choice) | Active links that feed a field on the destination service from the dive's own service are applied to a copy before `add_dive`; identity links (same unified attribute on both ends) are skipped; the site-name collapse in `to_unified` stays as the fallback | Composites and service-field links work for new dives too, tanks keep their names on upload as before, and the default board uploads exactly as it did. The plan's "drop the collapse" is withdrawn: with the site links `off` by default there would be no site name on new dives |
| 2026-09-21 | Template placeholder rules (phase 2 implementation choice) | Every placeholder must be one of the link's `source` fields, every source must appear in the template, a templated link is `to_target` or `off`, bare names resolve on the source service | Keeps the board's lines and the template in agreement; validation errors name the link |
| 2026-09-21 | Conflict resolution fetch | Adapters have no get-by-id; `resolve_conflict` fetches a two-day window around the dive and picks it by external id | Cheap on Divelogs (one list call); on Garmin it is three calls per dive in the window. Acceptable for a manual action |
...
2. ✅ **C3–C5, C18, C9** (2026-09-21) — the new conflict policies, conflicts queue, template renderer, CLI and scheduler wiring, and the profile export/import so links can move between machines from day one. Also the core of C20 (`test_mapping`). Left in C17: the unverified catalogue extras.
